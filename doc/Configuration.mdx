...
Configure flexible rules to meet any situation not covered by the [built-in rules](/docs/cli/rules/built-in-rules) by adding to the `rules` map in the Redocly configuration file.
...
|  --- | --- | --- |
| subject | [Subject object](#subject-object) | **REQUIRED.** Locates the specific [OpenAPI node type](#subject-node-types-and-properties) or `any` (see [example](#any-example)) and possible properties and values that the [lint command](/docs/cli/commands/lint) evaluates. Use with `where` to narrow further. |
...
| Property | Type | Description |
|  --- | --- | --- |
| type | string | **REQUIRED.** Locates the [OpenAPI node type](#subject-node-types-and-properties) that the [lint command](/docs/cli/commands/lint) evaluates. |
...
| ctx | `object` | `ctx` object extends the [Context object](/docs/cli/custom-plugins/custom-rules#the-context-object) with two properties: `baseLocation`, and `rawValue`. Base location (`baseLocation`) contains the location in the source document for current assertion. (See [Location Object](/docs/cli/custom-plugins/custom-rules#location-object)). Raw value is the original assertion value. |
...
| Property | Type | Description |
|  --- | --- | --- |
| message | `string` | [`string`] | Problem message that is displayed in the [lint command](/docs/cli/commands/lint) output. |
| location | `Location Object` | Location in the source document. See [Location Object](/docs/cli/custom-plugins/custom-rules#location-object) |
